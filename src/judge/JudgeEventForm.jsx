...
const JudgeEventForm = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get('http://localhost:5000/judge/my-events', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (updateMode) {
        ...
        const response = await axios.post('http://localhost:5000/judge/create', {
          title,
          description,
          date,
        },{
  ...
  const handleDelete = async (eventId) => {
    try {
      await axios.delete(`http://localhost:5000/judge/delete/${eventId}`, { withCredentials: true });
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:5000/judge/update/${updateEventId}`, {
        title,
        description,
        date,
      },{
